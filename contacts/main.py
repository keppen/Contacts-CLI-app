import learn
import click
from  learn import functions as foo
import os.path
from learn import interf
import sys
...
def create(what):
    ...
    path = os.path.abspath(learn.__file__)
    ...
    if what == 'contact'.lower():
        ...
        if l_n == 'q':
            sys.exit('Canceled')
        f_n = str(input('Enter first name : '))
        if f_n == 'q':
            sys.exit('Canceled')
        t_n = interf.get_int('Enter telefon number (mendatory): ')
        if t_n == 'q':
            sys.exit('Canceled')
        e_a = str(input('Enter e-mail adress: '))
        if e_a == 'q':
            sys.eixt('Canceled')
...
number or last name: ', where, conn)
        if entry['entry'] == '1':
            new_val = interf.get_any('Enter new last name: ')
            if new_val == 'q':
                sys.exit('Canceled')
        if entry['entry'] == '2':
            new_val = str(input('Enter new first name: '))
            if new_val == 'q':
                sys.exit('Canceled')
        if entry['entry'] == '3':
